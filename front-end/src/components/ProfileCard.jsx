...
export default function ProfileCard({ user }) {
    ...
    const handleSendMessage = async () => {
        ...
        setLoadingChat(true);
        try {
            // createChat returns { message, chat: {...} }
            const res = await createChat(user.id, null);
            if (res && res.chat) {
                navigate(`/dashboard/${authUser.id}/chat/${res.chat.id}`);
            }
        } catch (error) {
    ...
    return (
        <div className="w-full h-auto p-8 flex flex-col text-center border border-slate-700/50 rounded-3xl bg-[#0f172a]">
            ...
            <button
                ...
                className="w-full py-3 mb-6 bg-blue-500 hover:bg-blue-600 text-white font-semibold rounded-xl transition-colors flex items-center justify-center gap-2"
            >
                {loadingChat && <Loader2 className="animate-spin w-4 h-4" />}
                {loadingChat ? "Starting Chat..." : "Send Message"}
            </button>
